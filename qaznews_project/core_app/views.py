...
def home_page(request):
    ...
    last_posts = Post.objects.all().order_by('-created_at')[1:7]
...
def all_news_page(request):
    ...
    return request(request, './client/all-news.html', context)

def news_detail_page(request, pk):
    post = get_object_or_404(Post, pk=pk)
    tags = Tag.objects.all()
    last_posts = Post.objects.all().order_by('-create_at')[:3]
    ...
    return redirect(request, './client/news-detail.html', context)
